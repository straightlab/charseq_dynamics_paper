...
```{r}
plot_census_broadbinder<- function(deloc){
deloc %>%
  ...
  dplyr::filter(issig=="broad") %>%
  ggplot(aes(x=rna_type, y=n, fill=cell))+
   ...
   theme(legend.position = "right", legend.direction = "vertical", axis.text.x = element_text(angle=45, hjust=1)) +
    labs(x=NULL, y = "# of trans-delocalized RNAs") -> p
...
```{r, fig.width=2.6, fig.height=2.1}
mytypes  <- c('mRNA','lncRNA','snRNA','snoRNA')
#c("repeat","tRNAderived", "snRNAderived","cre","readthrough","antisense","intergenic")

mydata <- deloc_df %>% 
  dplyr::filter(annotation_type %in% c('exons'), FPM>1, deloc_score_type=='trans', ambiguous!=2, rna_type %in% mytypes) %>%
  mutate(rna_type = factor(rna_type, levels = (mytypes)), cell=factor(cell, levels=c('ES','DE'))) 

p <- plot_census_broadbinder(mydata)

fname="hist.num_trans_delocalized.exons.pdf"

p_fixed<- prettysave(p, here('figures/delocalization', fname), panel.width=1.2, panel.height=1.3)
plot_grid(p_fixed)
```
```{r, fig.width=3.6, fig.height=2.4}
mytypes  <- c("repeat","tRNAderived", "snRNAderived","cre","readthrough","antisense","intergenic")

mydata <- deloc_df %>% 
  dplyr::filter(annotation_type %in% c('intergenic'), FPM>1, deloc_score_type=='trans', ambiguous!=2, rna_type %in% mytypes) %>%
  mutate(rna_type = factor(rna_type, levels = (mytypes)), cell=factor(cell, levels=c('ES','DE'))) 

p <- plot_census_broadbinder(mydata)

fname="hist.num_trans_delocalized.intergenic.pdf"

p_fixed<- prettysave(p, here('figures/delocalization', fname), panel.width=1.2*7/4, panel.height=1.3)
plot_grid(p_fixed)
```
